...
class VentanaULTA(ctk.CTk):
    ...
    def crear_interfaz(self):
        ...
        btn_layout = ctk.CTkButton(layout_frame, text="📂 Seleccionar",
        ...
        btn_emmanuel = ctk.CTkButton(emmanuel_frame, text="📂 Seleccionar",
        ...
        btn_base = ctk.CTkButton(base_frame, text="📂 Seleccionar",
        ...
        btn_limpiar_layout = ctk.CTkButton(layout_frame, text="🗑 Limpiar",
                                   command=self.limpiar_layout,
                                   fg_color="#e74c3c", hover_color="#c0392b",
                                   font=BTN_FONT, width=80, height=BTN_HEIGHT, corner_radius=15)
        btn_limpiar_layout.pack(side="right", padx=(5,0))

        btn_limpiar_emmanuel = ctk.CTkButton(emmanuel_frame, text="🗑 Limpiar",
                                            command=self.limpiar_emmanuel,
                                            fg_color="#e74c3c", hover_color="#c0392b",
                                            font=BTN_FONT, width=80, height=BTN_HEIGHT, corner_radius=15)
        btn_limpiar_emmanuel.pack(side="right", padx=(5,0))

        btn_limpiar_base = ctk.CTkButton(base_frame, text="🗑 Limpiar",
                                        command=self.limpiar_base_general,
                                        fg_color="#e74c3c", hover_color="#c0392b",
                                        font=BTN_FONT, width=80, height=BTN_HEIGHT, corner_radius=15)
        btn_limpiar_base.pack(side="right", padx=(5,0))
        ...
        ctk.CTkLabel(footer_frame,
                    text="Sistema de V&C Generación de Tablas de Relación ULTA, v 4.0.0 © 2025",
    ...
    def cargar_emmanuel(self):
        archivo = filedialog.askopenfilename(
            title="Seleccionar Layout (Factura)",
            filetypes=[("Excel files", "*.xls *.xlsx")]
        )
        if archivo:
            try:
    ...
    def cargar_base_general(self):
        archivo = filedialog.askopenfilename(
            title="Seleccionar archivo Rel ULTA Embarques",
...
# --- Funciones para limpiar archivos ---
    def limpiar_layout(self):
        """Limpia la selección del archivo Layout"""
        self.layout = None
        self.lbl_layout_status.configure(text="No cargado", text_color=COLORES["gris_oscuro"])
        self.info_label.configure(text="❌ Layout limpiado")
        self.verificar_archivos_cargados()

    def limpiar_emmanuel(self):
        """Limpia la selección del archivo Emmanuel"""
        self.emmanuel = None
        self.lbl_emmanuel_status.configure(text="No cargado (Opcional)", text_color=COLORES["gris_oscuro"])
        self.info_label.configure(text="❌ Emmanuel limpiado")
        self.verificar_archivos_cargados()

    def limpiar_base_general(self):
        """Limpia la selección del archivo Base General y borra el JSON"""
        self.base_general = None
        self.lbl_base_status.configure(text="No cargado", text_color=COLORES["gris_oscuro"])

        # También eliminar archivo JSON de resources si existe
        json_path = os.path.join("resources", "base_general.json")
        if os.path.exists(json_path):
            os.remove(json_path)

        self.info_label.configure(text="❌ Base General limpiada")
        self.verificar_archivos_cargados()
    ...
    def generar_tabla(self):
        try:
            ...
            # --- Asignar Negaciones o Dictámenes ---
            if "NOM" in df_layout.columns and "Parte" in df_layout.columns:
                estatus_dict = {}

                # --- Agrupar las NOMs por código ---
                nom_por_codigo = {}
                for _, fila in df_layout.iterrows():
                    ...
                    if not codigo:
                        continue
                    if codigo not in nom_por_codigo:
                        nom_por_codigo[codigo] = set()
                    nom_por_codigo[codigo].add(nom_valor)

                # --- Reglas según combinaciones por código ---
                for codigo, noms in nom_por_codigo.items():
                    # CORREGIDO: Extraer solo el número principal de la NOM
                    nums = []
                    for n in noms:
                        # Buscar patrones como NOM-004, NOM-050, NOM-189
                        if "NOM-004" in n or "NOM-4" in n:
                            nums.append("4")
                        elif "NOM-050" in n or "NOM-50" in n:
                            nums.append("50")
                        elif "NOM-189" in n:
                            nums.append("189")
                        else:
                            # Para otras NOMs, extraer el primer número encontrado
                            match = re.search(r'NOM-(\d+)', n)
                            if match:
                                nums.append(match.group(1))

                    # ================================================================================= #
                    # DEBUG: Imprimir para verificar extraccion de numeros y asiganciones de negaciones
                    # print(f"Código: {codigo}, NOMs: {noms}, Números: {nums}")
                    # ================================================================================= #
                    
                    # --- Caso 1: NOM004 y NOM050 presentes ---
                    if "4" in nums and "50" in nums:
                        print(f"  ✅ Aplicando Caso 1 para código {codigo}")
                        estatus_dict[(codigo, "4")] = "ND"   # Negación de Dictamen
                        estatus_dict[(codigo, "50")] = "D"   # Dictamen

                    # --- Caso 2: NOM189 y NOM050 presentes ---
                    elif "189" in nums and "50" in nums:
                        print(f"  ✅ Aplicando Caso 2 para código {codigo}")
                        estatus_dict[(codigo, "189")] = "D"  # Dictamen
                        estatus_dict[(codigo, "50")] = "ND"  # Negación de Dictamen

                    else:
                        # Si no hay combinación especial, aplicar regla individual
                        print(f"  Aplicando reglas individuales para código {codigo}")
                        for nom_valor in noms:
                            # Determinar el número de NOM
                            if "NOM-004" in nom_valor or "NOM-4" in nom_valor:
                                nom_num = "4"
                                # Por defecto para NOM-004
                                estatus_dict[(codigo, "4")] = "ND"
                            elif "NOM-050" in nom_valor or "NOM-50" in nom_valor:
                                nom_num = "50"
                                # Por defecto para NOM-050
                                estatus_dict[(codigo, "50")] = "D"
                            elif "NOM-189" in nom_valor:
                                nom_num = "189"
                                # Por defecto para NOM-189
                                estatus_dict[(codigo, "189")] = "D"
                            else:
                                # Para otras NOMs, extraer número
                                match = re.search(r'NOM-(\d+)', nom_valor)
                                if match:
                                    nom_num = match.group(1)
                                    # Por defecto para otras NOMs
                                    estatus_dict[(codigo, nom_num)] = "D"
                                else:
                                    nom_num = ""
                                    estatus_dict[(codigo, nom_num)] = ""

                # ================================================================================= #
                # DEBUG: Mostrar el diccionario de estatus
                # print("Estatus dict:", estatus_dict)
                # ================================================================================= # 
                
                # --- Asignar estatus a cada fila ---
                estatus_final = []
                for _, fila in df_layout.iterrows():
                    codigo = str(fila.get("Parte", "")).strip()
                    nom_valor = str(fila.get("NOM", "")).upper().strip()
                    
                    # Determinar el número de NOM (misma lógica que arriba)
                    if "NOM-004" in nom_valor or "NOM-4" in nom_valor:
                        nom_num = "4"
                    elif "NOM-050" in nom_valor or "NOM-50" in nom_valor:
                        nom_num = "50"
                        ...
                        match = re.search(r'NOM-(\d+)', nom_valor)
                        nom_num = match.group(1) if match else ""
                    
                    # Buscar en el diccionario
                    clave = (codigo, nom_num)
                    estatus = estatus_dict.get(clave, "")
                    estatus_final.append(estatus)
                    
                    # DEBUG para filas específicas del Caso 2
                    if "189" in nom_valor and "50" in str(nom_por_codigo.get(codigo, [])):
                        print(f"🔍 DEBUG Caso 2 - Código: {codigo}, NOM: {nom_valor}, Estatus: {estatus}")

                tabla_relacion["ESTATUS"] = estatus_final
            ...
            def extraer_numero_nom(texto):
                if pd.isna(texto):
                    return ""
                # Buscar el número después de NOM-
                match = re.search(r'NOM-(\d+)', str(texto))
                return match.group(1) if match else ""

            if "NOM" in df_layout.columns:
                # CORREGIDO: Usar la nueva función de extracción
                tabla_relacion["CLASF UVA"] = df_layout["NOM"].apply(extraer_numero_nom)
            ...
            # --- CORREGIDO: ASIGNAR TIPO DE DOCUMENTO SEGÚN ESTATUS ---
            def determinar_tipo_documento(estatus, clasf, norma):
                try:
                    # PRIMERO: Si hay un estatus específico, usarlo (esto tiene prioridad)
                    if estatus == "ND":
                        return "ND"  # Negación de Dictamen
                    elif estatus == "D":
                        return "D"   # Dictamen
                    
                    # SEGUNDO: Si no hay estatus, aplicar reglas por defecto
                    clasf = int(clasf)
                    norma = int(norma)
                    
                    # Regla especial para NOM 4
                    if clasf == 4 and norma == 4:
                        return "ND"
                    else:
                        # Todas las demás NOM (como 50, 189, etc.)
                        return "D"
                except:
                    return "D"
            ...
            tabla_relacion["TIPO DE DOCUMENTO"] = tabla_relacion.apply(
                lambda fila: determinar_tipo_documento(
                    fila.get("ESTATUS", ""), 
                    fila["CLASF UVA"], 
                    fila["NORMA UVA"]
                ),
            ...
            # ================================================================================= #
            # DEBUG: Verificar resultados para Caso 2
            # print("\n=== VERIFICACIÓN CASO 2 ===")
            # ================================================================================= #

            # Verificar si las columnas existen antes de acceder a ellas
            columnas_necesarias = ["Parte", "NOM", "ESTATUS", "TIPO DE DOCUMENTO"]
            columnas_existentes = [col for col in columnas_necesarias if col in tabla_relacion.columns]

            if len(columnas_existentes) == len(columnas_necesarias):
                caso_2_filas = tabla_relacion[
                    (tabla_relacion["NOM"].astype(str).str.contains("189")) & 
                    (tabla_relacion["ESTATUS"] == "D")
                ]
                if not caso_2_filas.empty:
                    print("Filas con NOM-189 (deberían tener D):")
                    print(caso_2_filas[columnas_existentes].head(10))
                
                caso_2_nd_filas = tabla_relacion[
                    (tabla_relacion["NOM"].astype(str).str.contains("050|50")) & 
                    (tabla_relacion["ESTATUS"] == "ND")
                ]
                if not caso_2_nd_filas.empty:
                    print("\nFilas con NOM-050 (deberían tener ND):")
                    print(caso_2_nd_filas[columnas_existentes].head(10))
            else:
                print(f"Columnas faltantes: {set(columnas_necesarias) - set(columnas_existentes)}")
            ...
            if not df_base.empty and "CODIGO" in df_base.columns:
                ...
                def formatear_contenido_neto(valor):
                    """
                    Detecta valores con unidades g, ml, L, gr, combinaciones (2g/2ml, etc.)
                    y los convierte al formato NETO: <valor>.
                    """
                    if pd.isna(valor):
                        return ""
                    ...
                    # patrones de unidades válidas
                    patron_simple = r'\d+(\.\d+)?\s*(g|gr|ml|l|G|GR|ML|L)'
                    patron_combinado = r'\d+(\.\d+)?\s*(g|gr|ml|l)\s*/\s*\d+(\.\d+)?\s*(g|gr|ml|l)'

                    # Combinaciones ej: 2g/2ml, 5ML/2G
                    if re.search(patron_combinado, texto, flags=re.IGNORECASE):
                        return f"NETO: {texto}"

                    # Unidades simples ej: 250ml, 2g, 0.5 L
                    if re.search(patron_simple, texto, flags=re.IGNORECASE):
                        return f"NETO: {texto}"
                ...
                tabla_relacion["CONTENIDO"] = (
                    df_layout["PARTE_CLEAN"].map(dict_contenido).fillna("").apply(formatear_contenido_neto)
                )
